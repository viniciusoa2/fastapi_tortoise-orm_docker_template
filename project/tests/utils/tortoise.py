...
def register_tortoise(
    ...
    async def init_orm() -> None:  # pylint: disable=W0612
        await Tortoise.init(config=config, config_file=config_file, db_url=db_url, modules=modules, _create_db=True)
        logger.info("Tortoise-ORM started, %s", Tortoise.apps)
        if generate_schemas:
